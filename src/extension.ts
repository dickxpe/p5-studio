...
html, body {
  ...
  width: 100%;
  height: 100%;
  background: transparent; /* Keep webview transparent */
}
canvas.p5Canvas {
  display: block;
...
(function(){
  ...
  console.log = (...args) => { vscode.postMessage({type:'log', message:args}); origLog.apply(console,args); };
  const origErr = console.error;
  console.error = (...args) => { vscode.postMessage({type:'error', message:args.join(' ')}); origErr.apply(console,args); };
})();
...
if(typeof setup==='function'){const userSetup=setup; setup=function(){try{createCanvas(window.innerWidth, window.innerHeight); background(255); userSetup(); const c=get(0,0); window._p5Background=color(c[0],c[1],c[2],c[3]); clearError();}catch(e){showError('[Setup Error] '+(e.message||e)); throw e;}}}
...
window.addEventListener('resize', () => {
    if(window._p5Instance?._renderer){
        ...
        if(window._p5Background){ window._p5Instance.background(window._p5Background); }
        else { window._p5Instance.background(255); }
    }
});
