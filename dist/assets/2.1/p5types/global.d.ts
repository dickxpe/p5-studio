...
declare global {
     ...
     *   learn more about making sketches accessible.
     *
     *   textOutput() generates descriptions in English
     *   only. Text drawn with text() is not described.
     *   Shapes created with beginShape() are not
     *   described. WEBGL mode and 3D shapes are not
     *   supported.
     *
     *   Use describe() and describeElement() for more
     *   control over canvas descriptions.
     ...
     *   learn more about making sketches accessible.
     *
     *   gridOutput() generates descriptions in English
     *   only. Text drawn with text() is not described.
     *   Shapes created with beginShape() are not
     *   described. WEBGL mode and 3D shapes are not
     *   supported.
     ...
     *   the angles between which to draw the arc. By
    ...
    /**
     *   Stops adding vertices to a custom shape. The
     *   beginShape() and endShape() functions allow for
     *   creating custom shapes in 2D or 3D. beginShape()
     *   begins adding vertices to a custom shape and
     *   endShape() stops adding them.
     ...
     *   @return new p5.Element object.
     */
    function createRadio(containerElement?: object): p5.Radio;
     ...
     *   requestPointerLock() is active. But keep in mind
     *   that during an active pointer lock, mouseX and
     *   pmouseX are locked, so movedX is based on the
     *   MouseEvent's movementX value (which may behave
     *   differently in different browsers when the user is
     *   zoomed in or out).
     */
    let movedX: number;
     ...
     *
     *   The tenth and eleventh parameters, xAlign and
     ...
     *   coordinates. textureMode(NORMAL); // Apply the
     *   image as a texture. texture(img); // Draw the
     *   rectangle. beginShape(); // Top-left. // u: 0, v:
     *   0 vertex(0, 0, 0, 0, 0); // Top-right. // u: 1, v:
     *   0 vertex(30, 0, 0, 1, 0); // Bottom-right. // u:
     *   1, v: 1 vertex(30, 50, 0, 1, 1); // Bottom-left.
     *   // u: 0, v: 1 vertex(0, 50, 0, 0, 1); endShape();
     *
